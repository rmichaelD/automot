...
class VonBraun():
    ...
    def generate_obs(self, n):
        for i in range(n):
            ...
            if [x, y] != [self.unit.move_target[0], self.unit.move_target[1]]:
    ...
    def get_reward(self):
        ...
        self.d_mt = self.unit.get_distance()
        dist_comp = tolerance*self.d_init + self.d_init - self.d_mt
    ...
    def get_Dmt_grid(self):
        ...
        for i in range(Dmt_grid_size[0]):
            for j in range(Dmt_grid_size[1]):
                Dmt_grid_x[i,j] = (self.unit.x + i - self.v_range) - self.unit.move_target[0]
                Dmt_grid_y[i,j] = (self.unit.y + j - self.v_range) - self.unit.move_target[1]
